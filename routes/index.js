...
let allNEs = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, "../data/dumpEntitiesNCBIEntityTypes.json"),
    "utf8"
  )
);
let wtoNEs = []; // JSON.parse(fs.readFileSync(path.resolve(__dirname, "../data/dumpEntitiesWTO.json"), "utf8"));
let autoCompleteEntities = [].concat(geneVarNEs, allNEs, wtoNEs);
...
router.get("/getArticleMetadata/", (req, res) => {
  ...
  (async () => {
    ...
    try {
      result = await d3
        .sparql(process.env.SEMANTIC_INDEX_SPARQL_ENDPOINT, query)
        .then((data) => {
...
function getAutoCompleteSuggestions(input, entities) {
  ...
  let _includes = entities.filter((_entity) => {
    if (_count < process.env.SEARCH_MAX_AUTOCOMPLETE) {
      ...
      if (
        _entityLabLow.includes(input) &&
        !_startsWith.some(
          (_s) =>
            _s.entityLabel.toLowerCase() === _entityLabLow &&
            _s.entityUri === _entity.entityUri
        )
      ) {
        _count++;
        return true;
...
router.get("/autoComplete/", (req, res) => {
  ...
  let [startsWith, includes] = getAutoCompleteSuggestions(
    input,
    autoCompleteEntities
  );
  res
    .status(200)
    .json(startsWith.sort(sortDescCount).concat(includes.sort(sortDescCount)));
});
...
router.get("/searchDocuments/", (req, res) => {
  ...
  if (uri.length === 0) {
    ...
    let queryTpl = fs.readFileSync(
      "queries/searchDocumentByEntityTypes.sparql",
      "utf8"
    );
...
router.get("/searchDocumentsSubConcept/", async (req, res) => {
  ...
  if (uri.length === 0) {
    log.info(
      "searchDocumentsSubConcept - no parameter, returning empty response"
    );
    res.status(200).json({ result: [] });
  } else {
    ...
    uris.forEach((_uri) => {
      ...
      if (log.isDebugEnabled()) {
        log.debug(
          `searchDocumentsSubConcept - entityType for uri ${_uri}: ${_entityType}`
        );
      }
      ...
      switch (_entityType) {
        ...
        case "Variety":
          query = readTemplate(
            "searchDocumentBySubConceptVariety.sparql",
            _uri
          );
          break;
        default:
          log.warn(
            `searchDocumentsSubConcept - Unknown entityType for uri ${_uri}: ${_entityType}`
          );
          query = "";
      }
      ...
      if (query !== "") {
        if (log.isDebugEnabled()) {
          log.debug(
            "searchDocumentsSubConcept - Will submit SPARQL query: \n" + query
          );
        }
        ...
        let _promise = (async () => {
          ...
          try {
            _result = await d3
              ...
              .then((data) => {
                log.info(
                  "searchDocumentsSubConcept: query for uri " +
                    _uri +
                    " returned " +
                    data.length +
                    " results"
    ...
    Promise.allSettled(promises).then((_promises) => {
      ...
      _promises.forEach((_promise, index) => {
        if (index === 0) {
          ...
          joinedResults = joinedResults.filter((_r) =>
            _promise.value.some((_n) => _n.document === _r.document)
          );
          ...
          joinedResults = joinedResults.map((_r) => {
            let newResult = _promise.value.find(
              (_n) => _n.document === _r.document
            );
            let _matchedEntitiesAr = splitDollar(newResult.matchedEntities);
            _matchedEntitiesAr.forEach((_e) => {
              if (
                !_r.matchedEntities.some(
                  (_me) => _me.entityUri === _e.entityUri
                )
              )
        ...
        log.info(
          "searchDocumentsSubConcept: current number of results : " +
            joinedResults.length
        );
      });
      log.info(
        "searchDocumentsSubConcept: returning : " +
          joinedResults.length +
          " results"
      );
...
function getEntityTypeFromJson(uri) {
  try {
    let entityData = autoCompleteEntities.find(
      (_entry) => uri === _entry.entityUri
    );
